...
/*
 * Given a block and a desired shifting of that block, check to see if there are any obstructions in the way.
 *
 * If no obsctructions are present, move the block into new position.
 *
 * Only a change in x position is passed as a parameter since all movements include 1 step down no matter what.
 *
 * Rotations of the block should be performed before attempting movement.
 *
 * @returns 0 if the block was blocked from moving vertically, indicating that the block has "settled" and will no longer be interactive
*/
